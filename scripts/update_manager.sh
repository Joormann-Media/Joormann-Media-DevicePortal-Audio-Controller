...
print(json.dumps({
    ...
    "local_commit": local_commit, "remote_commit": remote_commit,
    "branch": branch, "ahead": ahead, "behind": behind,
    "restarted": restarted, "log": log,
}, ensure_ascii=False))
PY
}
...
remote_commit=""
ahead=0; behind=0; update_available=false
...
if git rev-parse --verify "$remote_ref" >/dev/null 2>&1; then
  ...
  if [[ "${behind}" =~ ^[0-9]+$ ]] && (( behind > 0 )); then update_available=true; fi
else
...
if [[ "$MODE" == "status" ]]; then
  msg="Up-to-date"
  if [[ "$update_available" == "true" ]]; then msg="Update vorhanden"; fi
...
if [[ -x "scripts/stop-all.sh" && -x "scripts/start-all.sh" ]]; then
  ...
  if bash scripts/service-restart.sh >>"$LOG_FILE" 2>&1; then restarted=true
  else log "[warn] service restart failed"; fi
fi
